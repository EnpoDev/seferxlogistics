...
<div class="p-6">
    ...
    <div class="flex items-center space-x-4 mb-6">
        ...
        <div>
            <h1 class="text-2xl font-bold text-black dark:text-white">{{ $restaurant->name }}</h1>
            <p class="text-sm text-gray-600 dark:text-gray-400">Restoran bilgilerini düzenleyin</p>
        </div>
    </div>

    <form action="{{ route('restoran.update', $restaurant) }}" method="POST" enctype="multipart/form-data" class="max-w-4xl">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="3"
                                class="w-full px-4 py-2.5 bg-gray-50 dark:bg-black border border-gray-200 dark:border-gray-700 rounded-lg text-black dark:text-white">{{ old('description', $restaurant->description) }}</textarea>
                        </div>
                ...
                <div class="bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-gray-800 rounded-xl p-6">
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="number" name="min_order_amount" value="{{ old('min_order_amount', $restaurant->min_order_amount) }}" step="0.01" min="0"
                                class="w-full px-4 py-2.5 bg-gray-50 dark:bg-black border border-gray-200 dark:border-gray-700 rounded-lg text-black dark:text-white">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-black dark:text-white mb-2">Teslimat Ücreti (₺)</label>
                            <input type="number" name="delivery_fee" value="{{ old('delivery_fee', $restaurant->delivery_fee) }}" step="0.01" min="0"
                                class="w-full px-4 py-2.5 bg-gray-50 dark:bg-black border border-gray-200 dark:border-gray-700 rounded-lg text-black dark:text-white">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-black dark:text-white mb-2">Max Teslimat (dk)</label>
                            <input type="number" name="max_delivery_time" value="{{ old('max_delivery_time', $restaurant->max_delivery_time) }}" min="1"
                                class="w-full px-4 py-2.5 bg-gray-50 dark:bg-black border border-gray-200 dark:border-gray-700 rounded-lg text-black dark:text-white">
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-6">
                <!-- Current Images -->
                @if($restaurant->logo || $restaurant->banner_image)
                <div class="bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-gray-800 rounded-xl p-6">
                    <h3 class="text-lg font-semibold text-black dark:text-white mb-4">Mevcut Görseller</h3>
                    <div class="space-y-4">
                        @if($restaurant->logo)
                        <div>
                            <p class="text-sm text-gray-500 mb-2">Logo</p>
                            <img src="{{ Storage::url($restaurant->logo) }}" alt="Logo" class="w-20 h-20 rounded-lg object-cover">
                        </div>
                        @endif
                        @if($restaurant->banner_image)
                        <div>
                            <p class="text-sm text-gray-500 mb-2">Banner</p>
                            <img src="{{ Storage::url($restaurant->banner_image) }}" alt="Banner" class="w-full h-24 rounded-lg object-cover">
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <!-- New Images -->
                <div class="bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-gray-800 rounded-xl p-6">
                    <h3 class="text-lg font-semibold text-black dark:text-white mb-4">Yeni Görsel Yükle</h3>
                    <div class="space-y-4">
                        <div>
                ...
                <div class="bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-gray-800 rounded-xl p-6">
                    ...
                    <div class="space-y-3">
                        <label class="flex items-center">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $restaurant->is_active) ? 'checked' : '' }} class="mr-3">
                            <span class="text-black dark:text-white">Aktif (Açık)</span>
                        </label>
                        <label class="flex items-center">
                            <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $restaurant->is_featured) ? 'checked' : '' }} class="mr-3">
