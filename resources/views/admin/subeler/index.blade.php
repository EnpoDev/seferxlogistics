...
<div class="animate-fadeIn" x-data="{ showCreateModal: false, showEditModal: false, editingBranch: null }">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-black dark:text-white">İşletmeler</h1>
            <p class="text-gray-600 dark:text-gray-400">Tüm işletmeleri yönetin</p>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-gray-50 dark:bg-black">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">İşletme</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/50">
                        ...
                        <td class="px-6 py-4">
                            @if($sube->branch)
                            <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-800">
                                {{ $sube->branch->name }}
                            </span>
                            @else
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end space-x-2">
                                ...
                                <form action="{{ route('admin.subeler.destroy', $sube) }}" method="POST" class="inline" onsubmit="return confirm('Bu işletmeyi silmek istediğinize emin misiniz?')">
                                    @csrf @method('DELETE')
                    ...
                    @empty
                    <tr><td colspan="5" class="px-6 py-12 text-center text-gray-500">İşletme bulunamadı</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div x-show="showCreateModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" x-transition>
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white dark:bg-[#1a1a1a] rounded-xl border border-gray-200 dark:border-gray-800 w-full max-w-md p-6">
                <h3 class="text-lg font-semibold text-black dark:text-white mb-4">Yeni İşletme</h3>
                <form action="{{ route('admin.subeler.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">İşletme Adı</label><input type="text" name="name" required class="w-full px-4 py-2 bg-gray-50 dark:bg-black border border-gray-200 dark:border-gray-700 rounded-lg text-black dark:text-white"></div>
    ...
    <div x-show="showEditModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" x-transition>
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white dark:bg-[#1a1a1a] rounded-xl border border-gray-200 dark:border-gray-800 w-full max-w-md p-6">
                <h3 class="text-lg font-semibold text-black dark:text-white mb-4">İşletme Düzenle</h3>
                <form :action="'/admin/subeler/' + (editingBranch?.id || '')" method="POST" class="space-y-4">
                    @csrf @method('PUT')
                    <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">İşletme Adı</label><input type="text" name="name" x-model="editingBranch.name" required class="w-full px-4 py-2 bg-gray-50 dark:bg-black border border-gray-200 dark:border-gray-700 rounded-lg text-black dark:text-white"></div>
