    ...
    <div class="p-6" x-data="{ billingPeriod: 'monthly' }">
        ...
        <div class="mt-8 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
            <div class="flex items-center justify-between">
                <div>
                    <p class="font-medium text-blue-700 dark:text-blue-400">Aktif Aboneliginiz: <strong>{{ $currentSubscription->plan->name }}</strong></p>
                    <p class="text-xs text-blue-600 dark:text-blue-500 mt-1">Sonraki odeme: {{ $currentSubscription->next_billing_date?->locale('tr')->isoFormat('D MMMM YYYY') ?? '-' }}</p>
                </div>
            </div>
        </div>
        @endif
